...
import { getPublicTripBundleByShareId, getPhotoUrl } from "@/lib/data";
import { buildPosterFrameSlots, getPosterLocationLabel, getPosterTripYear, isPosterComplete } from "@/lib/poster";
import { getPosterExportFormat, slugifyPosterFileLabel } from "@/lib/poster-export";
...
function getFormatLayout(formatId: string) {
  switch (formatId) {
    case "story":
      return {
        canvasPaddingX: 36,
...
function getPosterImageProxyUrl(requestUrl: string, sourceUrl: string) {
  const baseUrl = new URL(requestUrl);
  return `${baseUrl.origin}/api/poster-image?src=${encodeURIComponent(sourceUrl)}`;
}
...
export async function GET(request: Request, { params }: RouteProps) {
  try {
    ...
    const photoUrls = buildPosterFrameSlots(photos).map((photo) => {
      return photo ? getPosterImageProxyUrl(request.url, getPhotoUrl(photo)) : null;
    });
    ...
    const fileName = `${slugifyPosterFileLabel(trip.location) || "see-places-differently-poster"}-${format.fileSuffix}.png`;
