...
export async function generateMetadata({ params }: PublicPosterPageProps): Promise<Metadata> {
  ...
  const isCover = isCoverTripLike({ trip, mission });
  const title = `${trip.title} | Color Hunt`;
  const description = isCover
    ? `A public Color Hunt cover titled ${trip.title}, made from four photos and ready to share.`
    : `A public Color Hunt poster titled ${trip.title}, created in ${trip.location}. Hunt ${mission.color_name}, collect nine moments, and see the place differently.`;
...
export default async function PublicPosterPage({ params, searchParams }: PublicPosterPageProps) {
  ...
  const posterJsonLd = {
    ...
    name: trip.title,
    description: isCover
      ? `A public Color Hunt cover titled ${trip.title}, built from four photos.`
      : `A public Color Hunt poster titled ${trip.title}, built from nine ${mission.color_name} moments in ${trip.location}.`,
    ...
    <main className="app-shell page-frame">
      ...
      <div className="mx-auto max-w-5xl">
        <div className="mb-6 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm text-[var(--muted)]">Color Hunt public {isCover ? "cover" : "poster"}</p>
          <div className="flex flex-col gap-3 sm:flex-row">
            <SaveImageButton
              ...
              themeId={isCover ? coverThemeId : "classic"}
            />
            <ShareLinkButton
              shareId={shareId}
              url={shareUrl}
              title={`Color Hunt · ${trip.location}`}
              text={isCover ? `Made a cover for ${trip.title} with Color Hunt.` : `One place. One color. Nine moments. ${trip.location}`}
              fileUrl={exportUrls.post ?? null}
              buttonLabel={isCover ? "Share cover" : "Share poster"}
              buttonDescription={isCover ? "Share the finished cover from your phone’s native sheet" : "Share the finished poster from your phone’s native sheet"}
            />
            {!isCover ? (
            ...
            <PublicPosterCtaLink
              shareId={shareId}
              href={isCover ? "/covers/new" : challengeHref}
              label={isCover ? "Make your own cover" : `Start the ${challengeColor} challenge`}
            />
          </div>
        </div>

        <div className="mb-6 rounded-[1.5rem] border border-[rgba(47,97,223,0.12)] bg-[rgba(255,255,255,0.55)] p-4">
          <p className="eyebrow">{isCover ? "Make it yours" : "Challenge this poster"}</p>
          <p className="body-copy mt-2 text-sm sm:text-base">
            {isCover
              ? "Pick a style, add four photos, and turn your own camera roll into something worth posting."
